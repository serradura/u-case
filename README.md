...
Multiply.call(a: 2, b: 3)
# => #<Micro::Case::Result success? type=:ok data={ number: 6 }>

Multiply
  .call(a: 2, b: 'oops')
  .on_success { |r| puts r[:number] }
  .on_failure(:invalid_attributes) { |r| warn r[:errors] }
# warn: { "b" => "expected to be a kind of Numeric" }
```
...
class Multiply < Micro::Case
  ...
  def call!
    ...
    if a.is_a?(Numeric) && b.is_a?(Numeric)
      ...
      Failure result: { message: '`a` and `b` must be numeric' }
    end
  end
end
...
bad_result.data     # => { message: "`a` and `b` must be numeric" }
```
...
class Double < Micro::Case::Strict
  ...
  def call!
    Success result: { numbers: numbers.map { |n| n * 2 } }
  end
end
...
Use it when you want missing input to fail loudly instead of producing a use case whose `numbers` happens to be `nil`.
...
result.on_failure(:exception) do |data, use_case|
  ...
  when ZeroDivisionError then AppLogger.error(e.message)
...
Divide
  .call(a: 2, b: 0)
  .on_success { |r| puts r[:division] }
  .on_exception(TypeError)         { puts 'Please use only numeric attributes.' }
  .on_exception(ZeroDivisionError) { |_e| puts "Can't divide by 0." }
  .on_exception { |_e, _use_case|  puts 'Something went wrong.' }
# Can't divide by 0.
# Something went wrong.
```

> Both the typed `on_exception(ZeroDivisionError)` and the catch-all `on_exception` fire — like all u-case hooks, every match runs in declaration order (see [Result hooks](#result-hooks)).
...
class Divide < Micro::Case
  ...
  def call!
    return Failure(result: { invalid: invalid_attrs }) if invalid_attrs.any?
  ...
  private def invalid_attrs
    attributes.reject { |_k, v| v.is_a?(Numeric) }
  end
end

Divide.call(a: 2, b: 2).type   # => :ok
Divide.call(a: 2, b: '2').type # => :error
Divide.call(a: 2, b: 0).type   # => :exception
```
  ...
  def call!
    if a.is_a?(Numeric) && b.is_a?(Numeric)
      ...
      Failure :invalid_data, result: {
        attributes: attributes.reject { |_, v| v.is_a?(Numeric) }
      }
    end
  end
end

Multiply.call(a: 3, b: '2').type # => :invalid_data
```
...
class Double < Micro::Case
  ...
  def call!
    return Failure(:invalid,  result: { msg: 'number must be numeric' }) unless number.is_a?(Numeric)
    return Failure(:lte_zero, result: { msg: 'number must be > 0' })     if number <= 0
...
Double
  .call(number: 3)
  .on_success { |r| p r[:number] }                                  # 6
  .on_failure(:invalid)  { |r| raise TypeError,     r[:msg] }
  .on_failure(:lte_zero) { |r| raise ArgumentError, r[:msg] }

Double
  .call(number: -1)
  .on_failure { |_r, use_case| puts "#{use_case.class.name} failed" }  # 1. Double failed
  .on_failure(:lte_zero) { |r| raise ArgumentError, r[:msg] }          # 2. ArgumentError
```
...
Double
  ...
  .on_failure do |result, use_case|
    case result.type
    when :invalid  then raise TypeError,     'number must be numeric'
    when :lte_zero then raise ArgumentError, "`#{result[:number]}` must be > 0"
...
result
  .on_success { |r| accum += r[:number] }
  .on_success { |r| accum += r[:number] }
  .on_success(:ok) { |r| accum += r[:number] }
  .on_success(:ok) { |r| accum += r[:number] }
...
Double
  ...
  .on_failure do |(data, type), use_case|
    case type
    when :invalid  then raise TypeError,     'number must be numeric'
    when :lte_zero then raise ArgumentError, "`#{data[:number]}` must be > 0"
    else raise NotImplementedError
    end
  end
```
...
ForbidNegativeNumber.call(number: -1).then(Add3).failure? # => true
ForbidNegativeNumber.call(number:  1).then(Add3).data     # => { number: 4 }
```
...
Add.call(a: 2, b: 2).then   { |r| r.success? ? r[:sum] : 0 } # => 4
Add.call(a: 2, b: '2').then { |r| r.success? ? r[:sum] : 0 } # => 0
```
...
module Steps
  ...
  class Add2 < Micro::Case::Strict
    attribute :numbers
    def call!; Success result: { numbers: numbers.map { |n| n + 2 } }; end
  end
  ...
  class Square < Micro::Case::Strict
    attribute :numbers
    def call!; Success result: { numbers: numbers.map { |n| n * n } }; end
  end
end
...
Add2ToAllNumbers.call(numbers: %w[1 1 2 2 3 4]).data
# => { numbers: [3, 3, 4, 4, 5, 6] }
...
# => "numbers must contain only numeric types"
```
...
```ruby
DoubleAllNumbers        = Micro::Cases.flow([Steps::ConvertTextToNumbers, Steps::Double])
SquareAllNumbers        = Micro::Cases.flow([Steps::ConvertTextToNumbers, Steps::Square])
DoubleAllNumbersAndAdd2 = Micro::Cases.flow([DoubleAllNumbers, Steps::Add2])
SquareAllNumbersAndAdd2 = Micro::Cases.flow([SquareAllNumbers, Steps::Add2])

SquareAllNumbersAndAdd2
  .call(numbers: %w[1 1 2 2 3 4])
  .on_success { |r| p r[:numbers] } # => [3, 3, 6, 6, 11, 18]
```
...
class ConvertTextToNumber < Micro::Case
  attribute :text
  def call!; Success result: { number: text.to_i }; end
end

class ConvertNumberToText < Micro::Case
  attribute :number
  def call!; Success result: { text: number.to_s }; end
end
...
Double.call(text: '4').data[:text] # => "8"
```
...
DoSomeSum.call(a: 1, b: 2).data # => { sum: 6.5 }
```

Symbol-, method-, and lambda-based links all run **as the host use case**, so they report `class: DoSomeSum` in `result.transitions`. Only the `SumHalf` link (another use case class) contributes a transition with a different `use_case.class`. `accessible_attributes` grows as each link's `Success` output merges into the running data.
...
The host's internal transitions are interleaved with the outer flow's leaf transitions in execution order. If `DoSomeSum` produces 4 internal transitions and the outer flow has 2 other leaf steps, the final `result.transitions` has 6 entries.

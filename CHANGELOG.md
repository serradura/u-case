...
## [Unreleased]
### Added
- `Micro::Case::Result#then_expose` (alias `#then_return`) — projects a successful result's accumulated data down to a named subset of keys and re-tags it with a custom result `type`, in one call. Three call shapes: `then_expose(:user_created, [:user, :token])` (type + Array of keys), `then_expose(:user_created, :user)` (type + single Symbol key, normalized to `[:user]`), and `then_expose([:user, :token])` (Array-only, type defaults to `:data_exposed`). On a `Failure` result the call is a no-op and returns `self` unchanged — no new result is produced and `data` / `type` / `use_case` / `transitions` are preserved. Key lookup runs against the merged map `@__accessible_attributes.merge(@__accumulated_data)` (accumulated data wins on collisions), so attributes seen by earlier steps in the flow are exposable even when they don't appear in the immediate predecessor's `data`. The exposed slice is merged back into `@__accumulated_data`, so subsequent `then`-chained use cases still see the keys. Inspired by [`solid-result`'s](https://github.com/solid-process/solid-result/blob/main/lib/solid/output/success.rb) `Solid::Output::Success#and_expose`. Strictly additive — no existing public API behavior changes.
- `Micro::Case::Error::InvalidResultExposure` (subclass of `::KeyError`) — raised by `#then_expose` / `#then_return` when a requested key is absent from the merged `accessible + accumulated` map. The message lists the keys that **were** available (e.g. `key not found: :foo. Available to expose: :a, :b, :c`). Subclassing `::KeyError` is intentional: existing `rescue KeyError` clauses keep working unchanged.
- Argument validation on `#then_expose` raises `ArgumentError` up-front for ambiguous or malformed inputs: a bare Symbol (`then_expose(:a)`) is rejected because a single Symbol is ambiguous between "type with no keys" and "key with no type" — pass `then_expose([:a])` or `then_expose(:my_type, :a)` instead; empty key arrays (`[]`), non-Symbol key elements, and non-Symbol `type` in the two-arg form all raise with a clear message.
- READMEs (EN + pt-BR) document `#then_expose` / `#then_return` in lockstep under **Working with results**, including the three call shapes, the failure short-circuit, the missing-key error class, and an explicit note that `#then_expose` **does not** validate the new `type`/keys against the producing use case's `results { ... }` contract — the exposed shape is the seam's decision, not the producer's declaration (callers who want contract enforcement on chained shapes should `Success(...)` / `Failure(...)` explicitly inside `call!`).

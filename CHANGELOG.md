...
## [Unreleased]
### Added
- `Micro::Case::ActiveJob` — built-in async runner for use cases, conditionally loaded via `require 'micro/case/active_job'` (the gemspec is unchanged; `activejob` remains an optional, host-provided dependency). Closes #2. The new public surface:
  - **`Micro::Case.async(job_options: {}, raise_on_failure: false)`** and **`Micro::Case.later`** (alias) return a `Caller` whose `#call(input)` builds the per-use-case job subclass (lazily, named as `<UseCase>::Job` so ActiveJob can serialize it across enqueue/dequeue) and enqueues with the positional payload `[key, input, raise_on_failure]`. `Caller#to_proc` composes in `[…].each(&MyCase.async)` blocks.
  - **`Micro::Case.active_job(&block)`** class macro that evaluates a narrow DSL (`Micro::Case::ActiveJob::DSL`) against a per-use-case job subclass — eager, at class-body load time. DSL methods: `key`, `retry_on`, `discard_on`, `after_discard`, `default_options`, `around_perform`, `after_transaction_commit`. Unknown DSL methods raise `NoMethodError` with the valid-method list for typo protection.
  - **`Micro::Case::ActiveJob::Registry`** — thread-safe, `Mutex`-backed mapping of stable string keys to use-case classes. Auto-registers `klass.name => klass` on first `async`/`later` call so existing code adopts the runner without a DSL declaration. An explicit `active_job do; key 'namespace/use_case'; end` opts into a rename-stable key — payloads carry the key, so renaming the Ruby constant between enqueue and pickup no longer raises `NameError`. `register`, `fetch`, `key_for`, `all`, `clear`.
  - **`Micro::Case::ActiveJob::Caller`** — per-call enqueuer. Merges DSL `default_options` with explicit `async(job_options: …)`; explicit wins (matches Rails' `set` overriding `queue_as`). Permitted `job_options` keys: `:wait`, `:wait_until`, `:queue`, `:priority` (the only keys ActiveJob's `set` accepts).
  - **`Micro::Case::ActiveJob.batch(pairs)`** — bulk enqueue of `[Klass, input]` or `[Klass, input, raise_on_failure]` pairs. Uses `ActiveJob.perform_all_later([job_instance, …])` on Rails 7.1+, falls back to per-pair `enqueue` on older Rails. (Per the issue's open question, lives on `Micro::Case::ActiveJob` rather than `Micro::Case.async_batch`, to keep the `Micro::Case` class-method namespace tight.)
  - **`Micro::Case::ActiveJob::Error`** — raised on the worker when `raise_on_failure: true` and the use case returns `Failure(...)`. Exposes the underlying `Micro::Case::Result` via `#result` so apps can rescue and inspect.
  - **`Micro::Case::ActiveJob::UnknownKey`** — raised when a payload's key doesn't resolve in the registry (and, in non-strict mode, isn't a constantizable class name). Message names the offending key and the DSL line to add.
  - **`Micro::Case.config.strict_registry`** (default `false`) — when `true`, disables the `safe_constantize` fallback for registry misses. Friendlier defaults for early adopters; flip on for shops that want to guarantee no surprise constantization.
- ActiveJob features exposed declaratively via the DSL: `retry_on` / `discard_on` (Rails 5.1+), `after_discard` (Rails 7.1+; logs a one-time warning + no-op on older Rails), `enqueue_after_transaction_commit` via `after_transaction_commit :always | :never | :default` (Rails 7.2+; logs + no-ops on older Rails), `around_perform`, plus `default_options` for per-use-case `queue` / `priority` / `wait` defaults. GlobalID-coded inputs work transparently — the runner does not pre-coerce input on the worker side.
- Inheritance: a subclass of a use case that declared `active_job` inherits the job subclass reference, `default_options`, and registered key (so `Sub.async.call(input)` enqueues against the parent's identity). A subclass that redeclares `active_job do ... end` builds a fresh job subclass and must use a fresh key — the registry rejects duplicate registration to a different class.
- Eleven new internal check methods on `Micro::Case::Check` (`active_job_key!`, `active_job_retry_on!`, `active_job_discard_on!`, `active_job_after_discard!`, `active_job_around_perform!`, `active_job_default_options!`, `active_job_after_transaction_commit!`, `active_job_job_options!`, `active_job_batch_pairs!`, `active_job_registry_no_duplicate!`) so all argument validation introduced by the async runner participates in `config.disable_runtime_checks = true`. Each method exists on **both** `Check::Enabled` (raises) and `Check::Disabled` (no-op), matching the convention established in 5.4.0.
- `Appraisals` and `Gemfile`: `activejob` is added to each Rails appraisal's `:test` group and to the default `Gemfile`'s `:test` group (with `require: false`) so the new runner's test suite runs under `bundle exec rake test` and `bundle exec rake matrix`. The gemspec is unchanged — `activejob` remains an optional, host-provided dependency at runtime.

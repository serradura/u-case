...
class Micro::Case::Result::SuccessTest < Minitest::Test
  ...
  def test_nil_data_raises_the_curated_check_error
    assert_raises(Micro::Case::Error::InvalidResult) do
      Micro::Case::Result::Success.new(data: nil)
    end
  end

  def test_transitions_log_shape_matches_a_real_one_step_use_case
    result = Micro::Case::Result::Success.new(data: { x: 1 })
  ...
  def test_default_use_case_is_memoised_across_calls
    a = Micro::Case::Result::Success.new
    b = Micro::Case::Result::Success.new
  ...
  def test_to_yield_respects_the_type_filter
    wrapper = Micro::Case::Result::Success.to_yield(type: :created, data: { x: 1 })
